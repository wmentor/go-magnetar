...
internal/
  ...
  cmd/
    ...
    indexer/cmd.go               — indexer subcommand logic
    agent/cmd.go                 — agent subcommand logic
  tools/
...
cmd.Execute()
  ├── plugin.KongPlugins()     — collects CLI structs from CLIPlugin.RegisterCLI()
  ...
  └── ctx.Run(cfg)             — dispatches to the selected subcommand
```
